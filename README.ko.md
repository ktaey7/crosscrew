...
**402개 테스트가 통과**했습니다. 한 흐름의 성공을 모든 호스트 조합의 검증으로
확대해서 말하지는 않습니다.
...
작업 관리 코드, CLI 어댑터, 필요한 경로에서 사용하는 선택적 broker로 구성됩니다.
...
**Codex에서 Claude를 부르려면** host sandbox 밖의 broker가 필요합니다.
...
| **Antigravity (`agy`)** | 실험적 어댑터. 안정적인 fresh 세션과 자동 호스트 설치는 미지원. |
...
둘은 별도 프로젝트이며, Council 공개판의 기존 runner가 Crosscrew에 통합된 것은 아닙니다.

## 업데이트와 제거

진행 중인 작업을 먼저 마무리하세요. 코드·호스트 진입점·선택적 broker를 각각
